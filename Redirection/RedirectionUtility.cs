...
namespace InvertXMouse.Redirection
{
    public class RedirectionUtility
    {
        public static Dictionary<MethodInfo, RedirectCallsState> RedirectType(Type type, bool onCreated = false)
        {
            ...
            RedirectReversed(type, targetType, redirects, onCreated);
        ...
        private static void RedirectReversed(Type type, Type targetType, Dictionary<MethodInfo, RedirectCallsState> redirects, bool onCreated)
        {
            foreach (
                var method in
                type.GetMethods(BindingFlags.Public | BindingFlags.Static | BindingFlags.Instance | BindingFlags.NonPublic)
            .Where(method =>
            {
                var redirectAttributes = method.GetCustomAttributes(typeof(RedirectReversedAttribute), false);
                if (redirectAttributes.Length != 1)
                    return false;

                return ((RedirectReversedAttribute)redirectAttributes[0]).OnCreated == onCreated;
            }))
            {
        ...
        private static Tuple<MethodInfo, RedirectCallsState> RedirectMethod(Type targetType, MethodInfo detour, bool reverse)
        {
            ...
            var redirectCallsState = reverse ? RedirectionHelper.RedirectCalls(detour, originalMethod) : RedirectionHelper.RedirectCalls(originalMethod, detour);
